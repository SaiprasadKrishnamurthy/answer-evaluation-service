 ...
 */
@Service
class PhrasesScoreService(messagePublisher: MessagePublisher) : BaseScoreService(messagePublisher) {
    override fun getScore(questionAnswerMetadata: QuestionAnswerMetadata): Score {
        ...
        return if (questionAnswerMetadata.actualAnswer.isNotBlank()) {
            Score.zero(qmId = questionAnswerMetadata.id, answerType = AnswerType.expected, type = "phrases")
        } else {
            Score.zero(qmId = questionAnswerMetadata.id, answerType = AnswerType.actual, type = "phrases")
        }
    }
}