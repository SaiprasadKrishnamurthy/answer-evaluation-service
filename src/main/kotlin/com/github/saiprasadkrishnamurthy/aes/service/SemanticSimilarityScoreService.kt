 ...
 */
@Service
class SemanticSimilarityScoreService(messagePublisher: MessagePublisher) : BaseScoreService(messagePublisher) {
    override fun getScore(questionAnswerMetadata: QuestionAnswerMetadata): Score {
        ...
        return if (questionAnswerMetadata.actualAnswer.isNotBlank()) {
            Score.zero(qmId = questionAnswerMetadata.id, answerType = AnswerType.expected, type = "semantic")
        } else {
            Score.zero(qmId = questionAnswerMetadata.id, answerType = AnswerType.actual, type = "semantic")
        }
    }
}